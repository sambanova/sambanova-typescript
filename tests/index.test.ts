...
import { APIPromise } from 'sambanova/core/api-promise';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['SAMBA_NOVA_LOG'] = undefined;
  });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['SAMBA_NOVA_LOG'] = 'debug';
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['SAMBA_NOVA_LOG'] = 'not a log level';
    const client = new SambaNova({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'SAMBA_NOVA_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    ...
    test('in request options overridden by env variable', () => {
      process.env['SAMBA_NOVA_BASE_URL'] = 'http://localhost:5000/env';
      const client = new SambaNova({ apiKey: 'My API Key' });
